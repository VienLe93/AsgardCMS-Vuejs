...
@section('content-header')
    <h1>
        {{ trans('user::users.title.create user') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.user.user.index') }}">{{ trans('user::users.title.users') }}</a></li>
        <li class="active">{{ trans('user::users.title.create user') }}</li>
    </ol>
@stop
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('user::admin.users.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
